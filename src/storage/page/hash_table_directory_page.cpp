...
uint32_t HashTableDirectoryPage::GetGlobalDepthMask() { 
  return static_cast<uint32_t>(pow(static_cast<long double>(2), static_cast<long double>(GetGlobalDepth())) - 1); 
}

uint32_t HashTableDirectoryPage::GetLocalDepthMask(uint32_t bucket_idx) { 
  return static_cast<uint32_t>(pow(static_cast<long double>(2), static_cast<long double>(GetLocalDepth(bucket_idx))) - 1); 
}
...
void HashTableDirectoryPage::IncrGlobalDepth() {
  ...
  for(uint32_t i = old_size; i < new_size; i++){
    uint32_t old_index = i & static_cast<uint32_t>(pow(static_cast<long double>(2), static_cast<long double>(GetGlobalDepth()-1)) - 1);
    SetBucketPageId(i, GetBucketPageId(old_index));
    SetLocalDepth(i, GetLocalDepth(old_index));
  }
}
...
uint32_t HashTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx){
  uint32_t split_index = bucket_idx ^ GetLocalHighBit(bucket_idx);
  return split_index;
}

uint32_t HashTableDirectoryPage::Size() { 
  return static_cast<uint32_t> (pow(static_cast<long double>(2), static_cast<long double>(GetGlobalDepth()))); 
}

bool HashTableDirectoryPage::CanShrink() {
  //if all local depth is less than global depth, then can merge
...
uint32_t HashTableDirectoryPage::GetLocalHighBit(uint32_t bucket_idx) { 
  ...
  uint32_t high_bit = 1 << (local_depth - 1);
  return high_bit; 
}
